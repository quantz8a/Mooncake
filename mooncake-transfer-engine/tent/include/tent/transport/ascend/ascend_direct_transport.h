...
class AscendDirectTransport : public Transport {
   ...
   private:
    ...
    // HIXL AutoConnect refuses a session to the same engine name. Same-process
    // NPU↔host copies (Store put/get with memcpy disabled) use ACL memcpy.
    void completeLocalCopy(bool write, const std::vector<HixlTask*>& tasks);
    ...
    bool finishTaskLocked(HixlTask& task);
