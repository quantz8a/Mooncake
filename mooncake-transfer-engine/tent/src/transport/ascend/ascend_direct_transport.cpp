...
Status AscendDirectTransport::install(std::string& local_segment_name,
    ...
    CHECK_STATUS(initEngines());
    CHECK_STATUS(publishLocalEngines());
    installed_ = true;
...
Status AscendDirectTransport::initEngines() {
    ...
    for (const auto& [device_id, context] : devices) {
        ...
        auto init = WithHixlListenPort(init_options_, roce_listen);
        LOG(INFO) << "HIXL engine " << name
                  << " roce listen_port=" << roce_listen;
        CHECK_STATUS(engine->initialize(name, context, device_id, init));
...
void AscendDirectTransport::completeLocalCopy(
    bool write, const std::vector<HixlTask*>& tasks) {
    for (auto* task : tasks) {
        void* local = task->request.source;
        void* remote = reinterpret_cast<void*>(task->request.target_offset);
        void* dst = write ? remote : local;
        const void* src = write ? local : remote;
        auto ret = aclrtMemcpy(dst, task->request.length, src,
                               task->request.length, ACL_MEMCPY_DEFAULT);
        if (ret != ACL_ERROR_NONE) {
            LOG(ERROR) << "Local aclrtMemcpy failed, ret=" << ret
                       << ", errmsg: " << aclGetRecentErrMsg();
...
void AscendDirectTransport::startGroup(const std::string& remote_hixl,
    ...
    // Classic TE uses LocalCopyEngine when the dest ADXL/HIXL name is this
    // engine. Vendor HIXL AutoConnect rejects self ("Do not support
    // connection with self"). Fabric mem still goes through HIXL.
    if (!options_.use_fabric_mem &&
        remote_hixl == engines_[local_engine_idx]->name()) {
        completeLocalCopy(write, tasks);
        return;
    }
